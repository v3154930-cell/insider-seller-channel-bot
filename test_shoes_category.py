...
# The combo that seemed to work earlier: 15621048 + 91264
cat_id = 15621048
type_id = 91264
...
product = {
    "offer_id": f"shoes_{cat_id}_{type_id}",
    "name": "Sandal Test",
    "description": "Test",
    "category_id": cat_id,
    "type_id": type_id,
...
print(f"Sending: cat={cat_id} type={type_id} with {len(attributes)} required attrs")
    ...
    for i in range(25):
        ...
        if items:
            ...
            if status == "imported":
                ...
                print(f"Failed errors: {items[0].get('errors')}")
                break
    else:
        print("Timeout")
