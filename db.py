from parsers import is_legal_news
import os
import logging
import pytz
import sys
...
USE_TURSO = DATABASE_BACKEND in ("libsql", "turso") and bool(TURSO_DATABASE_URL and TURSO_AUTH_TOKEN)
...
def init_db():
    conn = _create_connection()
    logger.info(f"Database backend: {DATABASE_BACKEND}; turso_enabled={USE_TURSO}")
...
def _add_to_queue_batch_raw(items: List[Dict], seller_decisions: Dict[str, Dict] = None) -> int:
    if not items:
        return 0
    ...
    for item in items:
        try:
            ...
            # A digest item must be marked as in_digest only after successful digest publication.
            is_published = 0
            in_digest = 0
                 ...
                 ON CONFLICT(content_hash) DO UPDATE SET
                     ...
                     reason_tags = excluded.reason_tags,
                      ...
                      seller_relevance_score = CASE
                          WHEN news.is_published = 1 THEN news.seller_relevance_score
                          ELSE excluded.seller_relevance_score
                      END,
                      actionability_score = CASE
                          WHEN news.is_published = 1 THEN news.actionability_score
                          ELSE excluded.actionability_score
                      END,
                      in_digest = CASE
                          WHEN news.in_digest = 1 THEN 1
                          ELSE excluded.in_digest
                      END''',
                (
                    item.get('title', ''),
                    raw_text,
    ...
    if count > 0:
        try:
            ...
            row_pub = _fetch_one("SELECT COUNT(*) FROM news WHERE is_published = 0 AND seller_decision = 'publish'")
            logger.info(f"Publish pending after write: {row_pub[0] if row_pub else 0}")
        except Exception as e:
...
def clean_duplicates() -> int:
    ...
    try:
        ...
        for dup in duplicates:
            ...
            # Keep the first duplicate unchanged. Duplicate cleanup must not mark it as published.
...
def cleanup_by_retention_policy() -> Dict[str, int]:
    ...
    try:
        ...
        row = _fetch_one("SELECT COUNT(*) FROM news WHERE seller_decision = 'pending' AND is_published = 0 AND created_at < ?", (pending_cutoff,))
        pending_count = row[0] if row else 0
        if pending_count > 0:
            _execute("DELETE FROM news WHERE seller_decision = 'pending' AND is_published = 0 AND created_at < ?", (pending_cutoff,))
...
# === SELLER_FILTER_LIVE_WRAPPER_PATCH_V1 ===
def add_to_queue_batch(items, seller_decisions=None, *args, **kwargs):
    """
    Live seller filter wrapper.

    Нормальная боевая логика:
    - при записи новостей в БД прогоняем item через seller_filter.evaluate_item;
    - пишем seller_decision / seller_relevance_score / actionability_score;
    - обновляем уже существующие свежие дубли по link, чтобы они не застревали как digest/pending;
    - dry-run больше не является боевым режимом.

    Отключить можно только явно:
    SELLER_FILTER_LIVE=0
    """
    import os

    live_flag = str(os.getenv("SELLER_FILTER_LIVE", "1")).strip().lower()
    live_enabled = live_flag not in ("0", "false", "no", "off", "dryrun")
    lookback_hours = int(os.getenv("SELLER_FILTER_LIVE_LOOKBACK_HOURS", "8") or "8")

    items = list(items or [])
    decisions = dict(seller_decisions or {})

    if live_enabled and items:
        try:
            from seller_filter import evaluate_item
        except Exception as e:
            try:
                logger.warning("seller_filter_live import failed: %s", e)
            except Exception:
                pass
            evaluate_item = None
        ...
        if evaluate_item:
            for item in items:
                ...
                # Важно: если такая новость уже была вставлена раньше как digest/pending,
                # обновляем свежий дубль по link. Иначе publisher снова увидит pending loaded=0.
                try:
                    _execute(
                        """
                        UPDATE news
                        SET
                            ...
                            actionability_score = ?,
                            reason_tags = TRIM(COALESCE(reason_tags, '') || ' | seller_filter_live: ' || ?),
                            created_at = CASE
                        ...
                        WHERE link = ?
                          AND is_published = 0
                          AND (? = 'publish' OR datetime(created_at) >= datetime('now','localtime', ?))
                        """,
                        (decision, rel, act, reason, decision, link, decision, f"-{lookback_hours} hours"),
                    )
                    update_count += 1
                except Exception as e:
                    try:
                        logger.warning("seller_filter_live existing-row update failed link=%s err=%s", link[:120], e)
                    except Exception:
                        pass

        try:
            logger.info(
                "seller_filter_live applied items=%s decisions=%s existing_updates_attempted=%s lookback_hours=%s",
                len(items),
                live_count,
                update_count,
                lookback_hours,
