...
# === GIGACHAT FAILOVER PATCH ===
_previous_requests_post_for_gigachat = requests.post

def _gigachat_messages_to_text(messages):
    parts = []
...
def _requests_post_with_gigachat_failover(url, *args, **kwargs):
    response = _previous_requests_post_for_gigachat(url, *args, **kwargs)

    try:
        if response.status_code not in (429, 403, 402, 502, 503, 504):
            return response

        giga_key = os.getenv("GIGACHAT_AUTH_KEY")
        giga_scope = os.getenv("GIGACHAT_SCOPE", "GIGACHAT_API_PERS")
        giga_model = os.getenv("GIGACHAT_MODEL", "GigaChat-2-Lite")
        payload = kwargs.get("json")

        if not giga_key or not isinstance(payload, dict) or not payload.get("messages"):
            return response

        logger.warning(
            f"Primary LLM returned {response.status_code}, trying GigaChat fallback"
        )

        try:
            from gigachat import GigaChat
        except Exception as e:
            logger.warning(f"GigaChat SDK import failed: {e}")
            return response

        prompt_text = _gigachat_messages_to_text(payload.get("messages"))
        ...
        with GigaChat(
            ...
            giga_result = giga.chat(prompt_text)

        content = ""
        try:
            content = giga_result.choices[0].message.content
        except Exception:
            content = str(giga_result)
        ...
        logger.warning("GigaChat fallback HTTP: 200")
        return _FakeGigaResponse()

    except Exception as e:
        logger.warning(f"GigaChat fallback wrapper error: {e}")
        return response

requests.post = _requests_post_with_gigachat_failover
# === END GIGACHAT FAILOVER PATCH ===
...
SYSTEM_PROMPT = """Ты — редактор канала «Инсайдер Селлер» для продавцов на Ozon, Wildberries, Яндекс Маркете и других маркетплейсах.

Твоя задача — сделать короткий пост для MAX/Telegram: без воды, без канцелярита, с конкретным смыслом для селлера.
...
2–4 предложения: что произошло и почему это важно.

<b>🎯 Что это значит для селлера:</b> один конкретный вывод или действие.

Правила:
- Не используй markdown **жирный**, только HTML <b>...</b>.
- Не добавляй ссылку и строку «Источник:» — ссылку добавит код.
- Не пиши общие фразы вроде «это важно для отрасли», «следите за новостями», «важная информация для продавцов».
- Не используй универсальный вывод «проверьте цену, остатки, выплаты» без конкретики.
- Вывод должен зависеть от темы новости.

Матрица вывода:
- тарифы, комиссии, эквайринг: маржа, категории с низкой прибылью, риск ухода в минус;
- возвраты, логистика, склады: стоимость возврата, сроки вывоза, зависшие остатки;
- отзывы, рейтинг, карточки: слабые SKU, варианты товара, влияние на конверсию;
- налоги, НДС, регуляторы: будущая цена, юнит-экономика, ассортимент;
- ПВЗ и спорные операции: контроль возвратов, выдачи, ответственность за товар;
- API, кабинет, аналитика: интеграции, отчёты, остатки, автоматизация;
- история селлера, уход с маркетплейса, зависший товар: экономика не сходится, нужен план распродажи остатков, пересчёт слабых SKU и альтернативные каналы.

Пиши живым профессиональным языком. Максимум 700 символов."""
...
Сделай короткий пост в HTML-формате для MAX:
- Первой строкой поставь источник жирным: <b>📦 {source}</b>
- Затем пустая строка
- Заголовок сделай короче оригинала, без воды, и обязательно оберни его в <b>...</b>
- Затем 2-4 предложения: что произошло и почему важно
- Строку "🎯 Что это значит для селлера:" обязательно оберни в <b>...</b>, а сам вывод после двоеточия оставь обычным текстом
- НЕ добавляй ссылку, URL или строку "Источник:" внизу поста
- Ссылку на источник добавит код автоматически после текста
- Не используй markdown **жирный**, используй только HTML <b>...</b>
