...
/opt/newsbot_v2/venv/bin/python -c "import sqlite3; con=sqlite3.connect('/opt/newsbot_v2/news_queue.db'); cur=con.execute(\"UPDATE news SET seller_decision='digest' WHERE seller_decision='publish' AND COALESCE(is_published,0)=0 AND COALESCE(max_message_id,'')='' AND (COALESCE(seller_relevance_score,0)<3 OR COALESCE(actionability_score,0)<3)\"); con.commit(); print('preflight_demoted_weak_publish=',cur.rowcount); con.close()"
